...
M3U_FILE = "py/fofa/IPTV.m3u"  # 新增 M3U 路径

# ===============================
# 分类与映射配置 (此处已简化，请根据需要填回你的完整列表)
# 分类与映射配置
CHANNEL_CATEGORIES = {
    "央视频道": [
...
def save_run_count(count):
    with open(COUNTER_FILE, "w", encoding="utf-8") as f: f.write(str(count))

# ... (此处省略 first_stage, second_stage 的具体实现，保持你原来的代码逻辑) ...
...
def generate_m3u():
    ...
    with open(M3U_FILE, 'w', encoding='utf-8') as f:
        ...
        for line in lines:
            line = line.strip()
            if not line or "更新时间" in line: continue
            if "#genre#" in line:
                ...
                # 写入 M3U 格式
                f.write(f'#EXTINF:-1 group-title="{current_group}",{clean_name}\n{url_part.split("$")[0]}\n')
...
if __name__ == "__main__":
    ...
    # --- 关键修改：隔 5 次运行深度扫描 ---
    if run_count % 5 == 0:
        print(f"🚀 当前轮次 {run_count} 是 5 的倍数，开始全量更新 IPTV 文件...")
        second_stage() # 生成 zubo.txt
        third_stage()  # 检测并生成 IPTV.txt
        generate_m3u() # 同步生成 M3U
    else:
        print(f"ℹ️ 当前轮次 {run_count} 不是 5 的倍数，仅保留 IP 爬取成果。")
